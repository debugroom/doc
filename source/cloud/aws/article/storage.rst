...
Amazon CloudFrontは、キャッシュコントロールヘッダーを使用して、オリジンにファイルの更新バージョンがあるかどうかチェックする頻度を決定する。
失効期間を0秒に設定すると、Amazon CloudFrontはオリジンサーバーですべてのリクエストを再検証する。
キャッシュコントロールヘッダーが設定されていない場合、デフォルトでは、オリジンでファイルの変更があったことを最後に確認した時点から 24時間が経過した後に、
各エッジロケーションがリクエストを受け取ると、ファイルの更新バージョンの有無を゙チェックする。
